            ...
            <form action="{{route('catagory_add')}}" method="post" enctype="multipart/form-data">
             @csrf
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="branch" class="form-label">Select Branch Name</label>
                        <select class="form-select" aria-label="Default select example" name="branch_id" id="branch">
            ...
            <form action="{{route('catagory_update')}}" method="post" enctype="multipart/form-data">
             ...
             @method('PUT')
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="ubranch" class="form-label">Select Branch Name</label>
                        <select class="form-select" aria-label="Default select example" name="branch_id" id="ubranch" value="">
                        @foreach($branchs as $branch)
                        <option value="{{$branch->id}}">{{$branch->branch_name}}</option>
                        @endforeach
                        </select>
        ...
        <div class="modal-content">
            ...
            <form action="{{route('catagory_delete')}}" method="post">
             ...
             @method('delete')
                ...
                <input type="text" class="form-control" id="del_cat_id" name="deletingId">
                <div class="modal-footer">
        ...
        $(document).on('click', '.update_cat',function(){
            ...
            $.ajax({
                    ...
                    success: function(response){
                        ...
                        $('#ubranch').val(response.cat.branch_id);
